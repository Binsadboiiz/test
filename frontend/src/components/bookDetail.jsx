...
export default function BookDetail() {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("Rate API failed:", res.status, data);
      throw new Error((data && data.message) || `Failed to rate: ${res.status}`);
    }
    setBook(prev => {
      if (!prev) return prev;
    ...
    HandleErrorAPI(err, navigate, "BookDetail-Rate");
  }
};
      ...
      <div className="bookdetail-main-card">
        ...
        <div className="bookdetail-info">
          ...
          <div className="bookdetail-rating-row">
            <StarRating rating={book.averageRating || 0} onRate={handleRate} />
          ...
          <div className="bookdetail-actions">
            <button className="btn-primary-detail" onClick={handleAddFavorite}>
              ♥ Add to favorites
            </button>
            <button
                  ...
                  <div className="comment-avatar samill-avatar">
                    {c.user?.avatarUrl ? (
                      <img
                        ...
                        alt={
                          c.user?.displayname || c.user?.username || "avatar"
                        }
                        className="avatar-img"
                      />
                    ) : (
                      <span className="avatar-initial">
                        {(c.user?.displayname || c.user?.username || "U")
                          .charAt(0)
                          .toUpperCase()}
                      </span>
                    )}
